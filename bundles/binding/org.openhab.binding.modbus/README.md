...
The item configuration syntax binds the item to a specific coil, discrete input, input register or holding register. With 32bit value types, two registers are used to form the value of the item on read.
...
There is a variation in the simple item configuration format to allow to use different indices for reading and writing:
...
>[slaveName:writeIndex:trigger=TRIGGER,transformation=TRANSFORMATION,valueType=VALUETYPE]
 ```
Read and write entries can be combined, and there can be zero or more read/write entries. All the keyword arguments after index are optional. Defaults are such that they correspond to binding behaviour currently. Multiple read/write definitions can be specified with commas. 

Example of having multiple entries, separated with comma; for example:

```ini
<[slaveName:0],>[slaveName:0],>[slaveName:1]
 ```
(read from "index 0", and write to both "index 0" and "index 1")
...
Note that <i>first register</i> refers to register with address `start` (as defined in the slave definition), <i>second register</i> refers to register with address `start + 1` etc. The <i>index</i> refers to read index configured in the item, e.g. item `Switch MySwitch "My Modbus Switch" (ALL) {modbus="slave1:5"}` has 5 as read index.
