...
public class ModbusTCPTransport implements ModbusTransport {
    ...
    private void readFully(byte[] buffer, int off, int count) throws IOException, ModbusIOException {
        try {
            m_Input.readFully(buffer, off, count);
        } catch (EOFException e) {
            ModbusIOException e2 = new ModbusIOException("Premature end of stream (Header truncated).");
            e2.initCause(e);
            throw e2;
        }
    }
    ...
    public ModbusResponse readResponse() throws ModbusIOException {
        ...
        try {
            ...
            synchronized (m_ByteIn) {
                ...
                // read to byte length of message
                this.readFully(buffer, 0, 6);
                // extract length of bytes following in message
                int bf = ModbusUtil.registerToShort(buffer, 4);
                // read rest
                this.readFully(buffer, 6, bf);
                m_ByteIn.reset(buffer, (6 + bf));
                m_ByteIn.skip(7);
                int functionCode = m_ByteIn.readUnsignedByte();
                m_ByteIn.reset();
