...
public class ConfigUpdatedTestCase extends TestCaseSupport {
	...
	@Test
	public void testConfigUpdated() throws UnknownHostException,
		...
		for (int i = 0; i < 2; i++) {
			binding.updated(addSlave(newLongPollBindingConfig(), SLAVE_NAME,
					ModbusBindingProvider.TYPE_DISCRETE, null, 0, 2));
			// Let's not test connectionless UDP
			if (!serverType.equals(ServerType.UDP)) {
				waitForConnectionsReceived(i + 1);
			}
		}
