...
public class TestCaseSupport {
	 ...
	 */
	public static final ServerType[] TEST_SERVERS = new ServerType[] { 
//			ServerType.TCP,
//			ServerType.UDP, 
			ServerType.SERIAL
	};
	
	private static String SERIAL_SERVER_PORT = "/dev/ttyUSB0";
	private static String SERIAL_CLIENT_PORT = "/dev/ttyUSB1";
	
	private static SerialParameters SERIAL_PARAMETERS_CLIENT = new SerialParameters(
			SERIAL_CLIENT_PORT,
            115200,
	...
	private static SerialParameters SERIAL_PARAMETERS_SERVER = new SerialParameters(
			SERIAL_SERVER_PORT,
			SERIAL_PARAMETERS_CLIENT.getBaudRate(),
			SERIAL_PARAMETERS_CLIENT.getFlowControlIn(),
			SERIAL_PARAMETERS_CLIENT.getFlowControlOut(),
			SERIAL_PARAMETERS_CLIENT.getDatabits(),
			SERIAL_PARAMETERS_CLIENT.getStopbits(),
			SERIAL_PARAMETERS_CLIENT.getParity(),
			SERIAL_PARAMETERS_CLIENT.isEcho(),
            1000);
	static {
	...
	private static Dictionary<String, Object> addSlave(
			Dictionary<String, Object> config, String protocol,
			String connection, String slaveName, String type, String valuetype,
			int slaveId, int start, int length) {
		/**
		 * Add a modbus slave to config
		 */
		config.put(String.format("%s.%s.connection", protocol, slaveName),
				connection);
		config.put(String.format("%s.%s.id", protocol, slaveName),
				String.valueOf(slaveId));
		config.put(String.format("%s.%s.type", protocol, slaveName), type);
		if (valuetype != null) {
			config.put(String.format("%s.%s.valuetype", protocol, slaveName),
					valuetype);
		}
		config.put(String.format("%s.%s.start", protocol, slaveName),
				String.valueOf(start));
		config.put(String.format("%s.%s.length", protocol, slaveName),
	 ...
	 */
	protected Dictionary<String, Object> addSlave(
			Dictionary<String, Object> config, String slaveName, String type,
			String valuetype, int start, int length)
		...
		if (ServerType.TCP.equals(serverType)) {
			...
			protocol = "tcp";
			connection = String.format("%s:%d", localAddress()
					.getHostAddress(), port);
			...
			connection = String.format("%s:%d:%d:%s:%s:%s", 
					SERIAL_PARAMETERS_CLIENT.getPortName(),
					SERIAL_PARAMETERS_CLIENT.getBaudRate(),
					SERIAL_PARAMETERS_CLIENT.getDatabits(),
					SERIAL_PARAMETERS_CLIENT.getParityString(),
					SERIAL_PARAMETERS_CLIENT.getStopbitsString(),
					SERIAL_PARAMETERS_CLIENT.getEncoding());
		} else {
			throw new NotImplementedException();
		}
		
		return addSlave(config, protocol, connection, slaveName, type, valuetype,
				1, start, length);
	}
	...
	protected void configureSwitchItemBinding(int items, String slaveName, int itemOffset, String itemPrefix, State initialState)
		...
		for (int itemIndex = itemOffset; itemIndex < items + itemOffset; itemIndex++) {
			SwitchItem item = new SwitchItem(
					String.format("%sItem%d", itemPrefix, itemIndex + 1));
			if (initialState != null) {
				item.setState(initialState);
			}
			provider.processBindingConfiguration("test.items", item, String.format(
					"%s:%d", slaveName, itemIndex));
		}
	...
	protected void configureNumberItemBinding(int items, String slaveName, int itemOffset, String itemPrefix, State initialState)
			throws BindingConfigParseException {
		Assert.assertEquals(REFRESH_INTERVAL, binding.getRefreshInterval());
		final ModbusGenericBindingProvider provider = new ModbusGenericBindingProvider();
		for (int itemIndex = itemOffset; itemIndex < items + itemOffset; itemIndex++) {
	...
	protected void verifyBitItems(String expectedBits, int itemOffset, String itemPrefix) {
		for (int bitIndex = 0; bitIndex < expectedBits.length(); bitIndex++) {
			...
			verify(eventPublisher).postUpdate(
					String.format("%sItem%d", itemPrefix, bitIndex + itemOffset + 1), state);
		}
	}
	...
	protected void waitForRequests(int expectedRequestCount) {
		...
		while(waited < MAX_WAIT_REQUESTS_MILLIS){
			try {
				assertThat(modbustRequestCaptor.getAllReturnValues().size(),
	...
	protected void waitForConnectionsReceived(int expectedConnections) {
		...
		while(waited < MAX_WAIT_REQUESTS_MILLIS){
			try {
				if (ServerType.TCP.equals(serverType)) {
					verify(tcpConnectionFactory, times(expectedConnections))
							.create(any(Socket.class));
				} else if (ServerType.UDP.equals(serverType)) {
					verify(udpTerminalFactory, times(expectedConnections))
							.create(any(InetAddress.class), any(Integer.class));
				} else if (ServerType.SERIAL.equals(serverType)) {
					// No-op
				} else {
					throw new NotImplementedException();
	...
	private void startUDPServer() throws UnknownHostException,
	...
	private void startTCPServer() throws UnknownHostException,
			InterruptedException {
	...
	private void startSerialServer() throws UnknownHostException,
			InterruptedException {
