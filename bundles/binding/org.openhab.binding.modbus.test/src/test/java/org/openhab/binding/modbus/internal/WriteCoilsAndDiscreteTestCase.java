...
public class WriteCoilsAndDiscreteTestCase extends TestCaseSupport {	
	...
	private void setExpectedFailures() {
		boolean coilSameAsCommand = coilInitialValue ? Arrays.asList(
				ONE_COMMANDS).contains(command) : Arrays.asList(ZERO_COMMANDS)
				.contains(command);
		expectingAssertionError = ((ModbusBindingProvider.TYPE_COIL
				.equals(type) && coilSameAsCommand));
	}
	 ...
	 * @return
	 */
	private static ArrayList<Object[]> generateParameters(
			Object expectedValue, Command... commands) {
		ArrayList<Object[]> parameters = new ArrayList<Object[]>();
		for (ServerType serverType : TEST_SERVERS) {
			for (boolean discreteInputInitialValue : new Boolean[] { true,
					false }) {
						...
						for (Command command : commands) {
							for (int itemIndex : new Integer[] { 0, 1 }) {
								for (String type : new String[] {
										ModbusBindingProvider.TYPE_COIL,
										ModbusBindingProvider.TYPE_DISCRETE }) {
									parameters.add(new Object[] { serverType,
											discreteInputInitialValue,
											coilInitialValue, nonZeroOffset,
											type, itemIndex, command,
											expectedValue });
								}
							}
						}
	...
	public static Collection<Object[]> parameters() {
		ArrayList<Object[]> parameters = generateParameters(false,
				ZERO_COMMANDS);
	 ...
	 */
	public WriteCoilsAndDiscreteTestCase(ServerType serverType, boolean discreteInitialValue,
			boolean coilInitialValue, boolean nonZeroOffset, String type,
			int itemIndex, Command command, boolean expectedValue) {
	...
	public void testWriteDigitalsAfterRead() throws Exception {
		...
		int offset = (nonZeroOffset ? 1 : 0);
		binding.updated(addSlave(newLongPollBindingConfig(), SLAVE_NAME,
				type, null, offset, BIT_READ_COUNT));
		configureSwitchItemBinding(2, SLAVE_NAME, 0);
	...
	private void verifyRequests(boolean readRequestExpected) throws Exception {
		try {
			...
			if (readRequestExpected) {
				if (type == ModbusBindingProvider.TYPE_DISCRETE) {
					assertThat(requests.get(0),
							is(instanceOf(ReadInputDiscretesRequest.class)));
				} else if (type == ModbusBindingProvider.TYPE_COIL) {
					assertThat(requests.get(0),
							is(instanceOf(ReadCoilsRequest.class)));
				} else {
			...
			if (writeExpected) {
				assertThat(requests.get(expectedRequests - 1),
						is(instanceOf(WriteCoilRequest.class)));
				writeRequest = (WriteCoilRequest) requests
						.get(expectedRequests - 1);
				assertThat(writeRequest.getCoil(), is(equalTo(expectedValue)));
				assertThat(writeRequest.getReference(),
						is(equalTo(expectedDOIndex)));
			}
		} catch (AssertionError e) {
			if (expectingAssertionError) {
				System.err
						.println(String
								.format("Expected failure: discreteInitial=%s, coilInitial=%s, nonZeroOffset=%s, command=%s, itemIndex=%d, type=%s",
										discreteInitialValue, coilInitialValue,
										nonZeroOffset, command, itemIndex, type));
				return;
			} else {
				System.err
						.println(String
								.format("Unexpected assertion error: discreteInitial=%s, coilInitial=%s, nonZeroOffset=%s, command=%s, itemIndex=%d, type=%s",
										discreteInitialValue, coilInitialValue,
										nonZeroOffset, command, itemIndex, type));
		...
		if (expectingAssertionError) {
			System.err
					.println(String
							.format("Did not get assertion error (as expected): discreteInitial=%s, coilInitial=%s, nonZeroOffset=%s, command=%s, itemIndex=%d, type=%s",
									discreteInitialValue, coilInitialValue,
									nonZeroOffset, command, itemIndex, type));
			...
			System.err
					.println(String
							.format("OK: discreteInitial=%s, coilInitial=%s, nonZeroOffset=%s, command=%s, itemIndex=%d, type=%s",
									discreteInitialValue, coilInitialValue,
									nonZeroOffset, command, itemIndex, type));
		}
	}
	...
	private void initSpi() {
		dins = new DigitalIn[] { new SimpleDigitalIn(discreteInitialValue),
				new SimpleDigitalIn(discreteInitialValue),
				new SimpleDigitalIn(discreteInitialValue),
				new SimpleDigitalIn(discreteInitialValue) };
		for (DigitalIn din : dins) {
			spi.addDigitalIn(din);
		}
		douts = new DigitalOut[] { new SimpleDigitalOut(coilInitialValue),
				new SimpleDigitalOut(coilInitialValue),
				new SimpleDigitalOut(coilInitialValue),
				new SimpleDigitalOut(coilInitialValue) };
